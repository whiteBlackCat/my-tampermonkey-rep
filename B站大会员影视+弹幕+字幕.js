...
// @version      2.3
// @description  B站观影 弹幕 字幕替换 弹幕获取 历史弹幕
// @author       Polygon
...
function pageChange() {
  ...
  let addSubtitle = () => {
    ...
    subtitles.unshift({ content: '字幕加载成功[简体中文]', from: 0, location: 2, to: 0.5 })
    // 00:00
...
function changeMonth(message, downloadBtn) {
  let currentYear = parseInt(document.querySelector('#danmuYear').textContent)
  let currentMonth = parseInt(document.querySelector('#danmuMonth').textContent)
  let now = new Date()
  ...
  switch (this.getAttribute('class')) {
    case 'next-month':
      // 下个月
      if (now.getFullYear() == currentYear && currentMonth == now.getMonth() + 1) { break }
  ...
  // 重新赋值
  document.querySelector('#danmuYear').innerHTML = currentYear
  document.querySelector('#danmuMonth').innerHTML = currentMonth
  // 清空原有day
  let dayBox = document.querySelector('.day')
  dayBox.innerHTML = ""
  let weekStart = new Date(currentYear, currentMonth - 1, 1).getDay()
  let dayEnd = new Date(currentYear, currentMonth, 0).getDate()
  console.log(weekStart, dayEnd)
  let cursor = 0, day = 1
  let htmSTr = ""
  while (cursor < 42) {
    if (cursor < weekStart) {
      htmSTr += '<span class="which-day"></span>'
    } else {
      if (day <= dayEnd) {
        htmSTr += `<span class="which-day" day="${day}">${day}</span>`
        day++
      } else {
        htmSTr += '<span class="which-day"></span>'
      }
  ...
  dayBox.innerHTML = htmSTr
  // 查找支持
  let dayQuery = async (year, month) => {
    let cid = GM_getValue('cid')
    // 以当前时间向前查找
    let dateStr, api, data
  ...
  dayQuery(currentYear, currentMonth).then(supportDay => {
    supportDay.forEach((date) => {
      let day = parseInt(date.split('-')[2])
      let dayNode = document.querySelector(`.which-day[day="${day}"]`)
      dayNode.classList.add("support")
      dayNode.onclick = function () {
        if (this.classList.contains('active')) {
          this.classList.remove('active')
        } else {
          let days = document.querySelectorAll('.which-day.support')
          days.forEach((day) => {
...
let danmuDownload = (message, downloadBtn) => {
  ...
  let cid = GM_getValue('cid')
  // 获取日期
  let year = document.querySelector('#danmuYear').textContent
  let month = document.querySelector('#danmuMonth').textContent
  let day = document.querySelector('.which-day.support.active')
  if (day) { date = [year, ((parseInt(month) < 10) ? '0' : '') + month, ((parseInt(day.textContent) < 10) ? '0' : '') + day.textContent].join('-') }
  if (date) {
  ...
  if (xmlApi) {
    ...
    xmlPromise = fetch(protobufApi, { credentials: 'include' })
      ...
      .then(o => {
        ...
        let xmlStr = `<?xml version="1.0" encoding="UTF-8"?><i><chatserver>chat.bilibili.com</chatserver><chatid></chatid><mission>0</mission><maxlimit>${eles.length}</maxlimit><state>0</state><real_name>0</real_name><source>k-v</source>${xmlTagsStr}</i>`
        return xmlStr
      })
  }
  xmlPromise.then(s => {
...
let removeAllNode = (id) => {
  let Nodes = document.querySelectorAll(`[id^=${id}]`)
...
let clear = () => {
  removeAllNode('danmuDownloadBox')
  removeAllNode('switch')
  removeAllNode('diy_module_')
}
// 引入protobuf
if (document.URL.startsWith('https://www.bilibili.com/')) {
...
let initProtobuf = () => {
  ...
  downloadBox.setAttribute('class', 'danmuBox')
  downloadBox.setAttribute('id', 'danmuDownloadBox')
  downloadBox.innerHTML = `
  <div class="danmuTool">
      <span>
          <a id='danmuDownloadBtn'>下载弹幕</a>
      </span>
      <span id='danmuMore'>展开</span>
  </div>
  <div class="danmuContent" style="height: 0px">
      <div class="calendar">
          <div class="calendar-title-box">
              <div class="last-month">
                  <svg t="1625923239465" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2210"
                      width="20" height="20">
                      <path
                          ...
                          fill="#333333" p-id="2211"></path>
                  </svg>
              </div>
              <div id="year-month"><div><span id="danmuYear"></span><span>&nbsp;年</span></div><div><span id="danmuMonth"></span><span>&nbsp;月</span></div></div>
              <div class="next-month">
                  <svg t="1625923704105" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1494"
                      width="20" height="20">
                      <path
                          ...
                          fill="#333333" p-id="1495"></path>
                  </svg>
              </div>
          </div>
          <div class="calendar-date">
              <div class="week">
                  <span class="which-week">日</span>
                  <span class="which-week">一</span>
                  <span class="which-week">二</span>
                  <span class="which-week">三</span>
                  <span class="which-week">四</span>
                  <span class="which-week">五</span>
                  <span class="which-week">六</span>
              </div>
              <div class="day">
              </div>
          </div>
  ...
  `
  document.querySelector(bangumi ? '.plp-r' : '.r-con').insertBefore(downloadBox, document.querySelector('#danmukuBox'))
  let downloadBtn = document.querySelector('#danmuDownloadBtn')
  // 上一个月 下一个月按钮
  document.querySelector('.last-month').onclick = function () { changeMonth.call(this, message, downloadBtn) }
  document.querySelector('.next-month').onclick = function () { changeMonth.call(this, message, downloadBtn) }
  document.querySelector('.next-month').click()
  downloadBox.querySelector('#danmuMore').onclick = function () {
    let contentBox = document.querySelector('.danmuContent')
    if (this.textContent == '展开') {
      contentBox.style.height = 'calc(93px + 320px / 7 * 6)'
      this.textContent = '收起'
    } else {
  ...
  GM_addStyle(`
      ...
      .calendar-title-box {
          ...
          padding: 0px 80px;
          height: 50px;
          /* border: 1px solid rgb(204, 74, 74); */
      }
      .calendar-date {
          width: 320px;
          align-items: center;
      ...
      span.which-day.support {
          color: black;
      }
      span.which-day.active {
...
const origin = [
  { regex: 'barrage', api: 'https://www.mtosz.com/m3u8.php?url=', color: '#f514b9' },
  { regex: 'dmku', api: 'https://vip.parwix.com:4433/player/?url=', color: '#ff6429' }
]

const bangumi = location.href.search('bangumi') !== -1
...
if (!document.URL.startsWith('https://www.bilibili.com/')) {
  ...
  if (!document.URL.startsWith('https://www.bilibili.com/')) {
    ...
    let stop = setInterval(() => {
      if (!maxN) { clearInterval(stop) }
      btn = document.querySelector('[class*="player-play-icon"]')
      if (btn) {
        clearInterval(stop)
